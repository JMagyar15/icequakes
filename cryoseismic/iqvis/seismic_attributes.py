...
def __make_catalogues(events, stream, events_list, stream_list, starttime, endtime, signal_type='amplitude', thr_travel_time=0, thr_coincidence_sum=1):
    ...
    if len(events) > 0:
        ...
        events_df = pd.DataFrame(columns=['event_id', 'stations', 'network_time', 'ref_time', 'ref_duration', 'ref_amplitude', 'ref_energy','signal_type'])
        traces_df = pd.DataFrame(columns=['event_id', 'stations', 'network_time', 'time', 'duration', 'amplitude', 'energy','signal_type'])
        return events_df, traces_df
...
def get_attributes(events, stream, *attributes):
    ...
    for i in range(0, len(events)):
        ...
        #work out the start and end time of the event from the event or trace catalogue
        try:
        ...
        for j in range(0, len(component_list)): #loop over the stations
            component_stream = component_list[j].slice(start_time, end_time) #get the stream for the j-th station
            print(component_stream)
            # add attribute names and values to lists
            if len(component_stream) == 0:
                break #this is missing data so do not try and compute attributes
            else:
                #stream is not empty, but may having missing data in masked arrays
                bool_miss = np.array([isinstance(tr.data,np.ma.MaskedArray) for tr in component_stream])
                print(bool_miss)
                if bool_miss.max():
                    break
            try:
...
def plot_durations():
    """
    Histogram of the durations of the detected events.
    """
    return

def plot_times():
    """
    Histogram of the times of events relative to midnight.
    """
    return

def station_distances(stream,starttime,station_name='stations',thr_coincidence_sum=1):
    """
    Private function for get_events() to calcualte maximum distance between any seismometer in a given array and its closest (n-1) neighbours.
    """
    ...
    for i in range(0, len(stream)):
        ...
        if i == 0 or not np.any(location == np.asarray(seismometer_list)):
            try:
                inv = read_inventory(filename)
                coordinates = inv.get_coordinates(stream[i].id) #! removed the starttime input as we have given filename in the read_inventory part
                coordinates['location'] = location
                # append to lists
                coordinates_list.append(coordinates)
                seismometer_list.append(coordinates['location'])
